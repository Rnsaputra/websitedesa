...
<x-body>
            ...
            <div class="flex flex-1">
                ...
                <div class="  w-full static flex flex-col gap-4     ">
                    ...
                    <div class="flex gap-3 min-h-20 bg-white p-2 shadow-xl lg:px-15 w-full items-center mt-15">
                        <h1 class="w-full font-bold text-4xl block ">User List</h1>
                        <div class="flex gap-2 items-center">
                            <a href="user/create"
                                ...
                                <i class="fa-solid fa-plus fa-lg p-2"></i>Tambah User
                            </a>
                        </div>
                    </div>
                            ...
                            <table class="min-w-full bg-white border border-gray-200">
                                <thead>
                                    <tr class="bg-rose-500 text-white">
                                        <th class="py-2 px-4 border-b text-left border-r border-gray-200">Name</th>
                                        <th class="py-2 px-4 border-b text-left border-r border-gray-200">username</th>
                                        <th class="py-2 px-4 border-b text-left border-r border-gray-200">Email</th>
                                        <th class="py-2 px-4 border-b text-left border-r border-gray-200">Role</th>
                                ...
                                <tbody>
                                    @foreach ($user as $users)
                                        <tr class="{{ $loop->even ? 'bg-rose-100' : 'bg-white' }}">
                                            ...
                                            <td class="py-2 px-4 border-r border-gray-200">{{ $users->username }}</td>
                                            <td class="py-2 px-4 border-r border-gray-200">{{ $users->email }}</td>
                                            <td class="py-2 px-4 border-r border-gray-200 "><span
                                                    class="{{ $users->role == 'admin' ? 'bg-green-200' : 'bg-yellow-200' }}  p-1 rounded-lg">{{ $users->role }}</span>
                                            </td>
                                            <td class="py-2 px-4 border-r border-gray-200 flex gap-2">

                                                <a href="/dashboard/admin/user/{{ $users->username }}/edit" class="block">
                                                    <div
                                                ...
                                                <form action="/dashboard/admin/user/{{ $users->username }}" method="post">
                                                    @method('delete')
                                                    @csrf

                                                    <button onclick="return confirm('anda yakin ingin menghapus {{ $users->name }}')"">
                                                        <div
    ...
    <script>
        const side = document.querySelector('#nav-users');
